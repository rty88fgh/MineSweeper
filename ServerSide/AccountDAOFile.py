...
class AccountDAOFile(DAOBase):
    PLAYER_INFO_FILE = "Account.json"

    def Insert(self, name, password):
        accounts = json.load(open(AccountDAOFile.PLAYER_INFO_FILE, "r"))
        ...
        fp = open(AccountDAOFile.PLAYER_INFO_FILE, "w+")
        fp.write(json.dumps(accounts))
        return account

    def FindAll(self):
        return json.load(open(AccountDAOFile.PLAYER_INFO_FILE, "r"))
