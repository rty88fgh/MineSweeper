...
class Computer(Player):
    ...
    def ComputerAction(self, grids):
        ...
        if len(set_flag) > 0:
            ...
            return computerActionMap["Flag"], (select_grid["X"], select_grid["Y"])

        elif len(must_not_mine) > 0:
            select_grid = must_not_mine[0]
            return computerActionMap["ClickGrid"], (select_grid["X"], select_grid["Y"])
            ...
            return computerActionMap["Flag"] if nonclicked_grid[0][1] == float(1) else \
                   computerActionMap["ClickGrid"], nonclicked_grid[0][0]
